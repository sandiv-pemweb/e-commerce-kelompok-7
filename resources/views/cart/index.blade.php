<x-store-layout>
    <div class="bg-brand-gray min-h-screen py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h1 class="text-3xl font-serif font-bold text-brand-dark mb-8">Shopping Cart</h1>
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            @foreach($cartByStore as $storeId => $items)
                                @php
                                    ...
                                    $storeTotal = $items->sum(function($cart) { return $cart->subtotal; });
                                @endphp
                                <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
                                    <!-- Store Header -->
                                    <div class="bg-gray-50 px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                                        <div class="flex items-center gap-3">
                                            <div class="w-8 h-8 rounded-full bg-brand-orange/10 flex items-center justify-center text-brand-orange font-bold text-sm">
                                        ...
                                        <input type="checkbox" name="stores[]" value="{{ $storeId }}" class="store-checkbox rounded border-gray-300 text-brand-orange focus:ring-brand-orange" checked>
                                    </div>
                                            ...
                                            <div class="p-6 flex flex-col sm:flex-row gap-6">
                                                ...
                                                <div class="w-full sm:w-24 h-24 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                                                    @if($cart->product->productImages->count() > 0)
                                                        <img src="{{ $cart->product->productImages->first()->image_url }}" alt="{{ $cart->product->name }}" class="w-full h-full object-cover">
                                                    @endif
                                                </div>
                                                ...
                                                <div class="flex-1">
                                                    <div class="flex justify-between items-start mb-2">
                                                        <h4 class="font-bold text-brand-dark text-lg">
                                                            <a href="{{ route('products.show', ['store' => $cart->product->store->slug, 'product' => $cart->product->slug]) }}" class="hover:text-brand-orange transition-colors">
                                                                {{ $cart->product->name }}
                                                            </a>
                                                        </h4>
                                                    ...
                                                    <div class="flex items-center justify-between">
                                                        ...
                                                        <!-- Remove Button -->
                                                        <button onclick="removeFromCart(this, {{ $cart->id }})" class="text-gray-400 hover:text-red-500 transition-colors flex items-center gap-1 text-sm font-medium">
                            ...
                            <div class="bg-white rounded-xl shadow-sm p-6 sticky top-8 border border-gray-100">
                                ...
                                <div class="space-y-4 mb-6">
                                    <div class="flex justify-between text-gray-600">
                                        ...
                                        <span id="selectedStoresCount" class="font-bold text-brand-dark">{{ $cartByStore->count() }}</span>
                                    </div>
                                    <div class="flex justify-between text-gray-600">
                                        <span>Total Items</span>
                                        <span id="totalItems" class="font-bold text-brand-dark">{{ $cartByStore->sum(function($items) { return $items->count(); }) }}</span>
                                    </div>
                                </div>
                                ...
                                <div class="border-t border-gray-100 pt-6 mb-8">
                                    <div class="flex justify-between items-end">
                                        ...
                                        <span class="text-2xl font-bold text-brand-orange" id="grandTotal">Rp {{ number_format($grandTotal, 0, ',', '.') }}</span>
                                    </div>
                                    <p class="text-xs text-gray-400 mt-2 text-right">*Shipping calculated at checkout</p>
                                </div>

                                <button type="submit" class="w-full bg-brand-dark text-white px-6 py-4 rounded-xl hover:bg-brand-orange transition-colors font-bold text-lg shadow-lg shadow-brand-orange/10 flex items-center justify-center gap-2">
                                    Proceed to Checkout
                                ...
                                <a href="{{ route('products.index') }}" class="block w-full text-center mt-4 text-gray-500 hover:text-brand-dark font-medium transition-colors">
                                    Continue Shopping
                                </a>
                            </div>
                        </div>
            ...
            @else
                <div class="bg-white rounded-xl shadow-sm p-16 text-center border border-gray-100">
                    ...
                    <h3 class="text-xl font-bold text-brand-dark mb-2">Your cart is empty</h3>
                    <p class="text-gray-500 mb-8">Looks like you haven't added any books yet.</p>
                    <a href="{{ route('products.index') }}" class="inline-flex items-center justify-center px-8 py-3 bg-brand-orange text-white font-bold rounded-xl hover:bg-brand-dark transition-colors shadow-lg shadow-brand-orange/20">
                        Start Shopping
                    </a>
                </div>
            @endif
