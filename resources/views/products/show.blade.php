                    ...
                    <div class="lg:col-span-4">
                        <div class="relative aspect-[3/4] rounded-2xl overflow-hidden bg-gray-100 shadow-lg group">
                            @if($product->productImages->count() > 0)
                                <img id="mainImage" src="{{ $product->productImages->first()->image_url }}"
                                    alt="{{ $product->name }}"
                        ...
                        @if($product->productImages->count() > 1)
                            <div class="flex gap-3 mt-4 overflow-x-auto pb-2">
                                @foreach($product->productImages as $image)
                                    <button
                                        class="flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 border-transparent hover:border-brand-orange transition-all focus:outline-none focus:border-brand-orange"
                        ...
                        <div class="flex justify-between items-start mb-4">
                            ...
                            <button onclick="toggleWishlist(this, {{ $product->id }})"
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    fill="{{ Auth::user() && Auth::user()->wishlists->contains('product_id', $product->id) ? 'currentColor' : 'none' }}"
                        ...
                        <div class="flex items-center gap-6 mb-8">
                            <div class="flex items-center gap-2">
                                <div class="flex text-brand-orange">
                                    @for($i = 1; $i <= 5; $i++)
                                        <svg class="w-5 h-5 {{ $i <= round($averageRating) ? 'fill-current' : 'text-gray-300' }}"
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 border-y border-gray-100 py-6">
                            <div>
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <span
                                        class="font-bold text-brand-dark text-sm">{{ $product->author ?? $product->store->name }}</span>
                                </div>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <a href="{{ route('stores.show', $product->store->slug) }}"
                                        class="font-bold text-brand-dark text-sm hover:text-brand-orange hover:underline">
                                        {{ $product->store->name }}
                                    </a>
                                </div>
                            </div>
                        ...
                        <div class="mt-auto">
                            <div class="flex items-center gap-4 mb-8">
                                @if($product->discount_price > 0)
                                    <span
                                        class="text-2xl md:text-3xl font-bold text-brand-dark">{{ $product->formatted_discount_price }}</span>
                                    <span class="text-lg text-gray-400 line-through decoration-red-500 decoration-2">
                                        {{ $product->formatted_price }}
                                    </span>
                                    <span
                                        class="px-2 py-1 bg-red-100 text-red-600 text-xs font-bold rounded">-{{ $product->discount_percentage }}%</span>
                                @else
                                    <span
                                        class="text-2xl md:text-4xl font-bold text-brand-dark">{{ $product->formatted_price }}</span>
                                @endif
                            </div>
                                    ...
                                    @if($product->stock > 0)
                                        ...
                                        <div
                                            ...
                                            <button type="button" id="decrement-button"
                                                class="px-4 py-2 text-brand-orange text-xl font-bold hover:bg-gray-50 rounded-l-lg">-</button>
                                            <input type="number" name="quantity" id="quantity-input" value="1" min="1"
                                                max="{{ $product->stock }}"
                                                class="w-full md:w-12 text-center border-0 focus:ring-0 font-bold text-brand-dark p-0">
                                            <button type="button" id="increment-button"
                                                class="px-4 py-2 text-brand-orange text-xl font-bold hover:bg-gray-50 rounded-r-lg">+</button>
                                        </div>

                                            <button type="button" onclick="addToCart({{ $product->id }}, document.getElementById('quantity-input').value)" class="w-full md:flex-1 bg-white border border-brand-orange text-brand-orange px-8 py-3 rounded-xl hover:bg-orange-50 transition-all font-bold text-lg flex items-center justify-center gap-2">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
                                                </svg>
                                                Tambah ke Keranjang
                                            </button>
                                            <button type="button" onclick="addToCart({{ $product->id }}, document.getElementById('quantity-input').value, true)" class="w-full md:flex-1 bg-brand-orange text-white px-8 py-3 rounded-xl hover:bg-brand-dark transition-all font-bold text-lg shadow-lg shadow-brand-orange/20 flex items-center justify-center gap-2">
                                                Beli Sekarang
                                            </button>
                                    @else
                        ...
                        <div x-show="activeTab === 'details'" x-transition:enter="transition ease-out duration-300"
                            ...
                            <div class="grid grid-cols-1 gap-y-4">
                                <div class="grid grid-cols-3 py-3 border-b border-gray-50">
                                    <span class="font-bold text-brand-dark">Judul Buku</span>
                                ...
                                <div class="grid grid-cols-3 py-3 border-b border-gray-50">
                                    ...
                                    <span class="col-span-2 text-gray-500">
                                        <a href="{{ route('stores.show', $product->store->slug) }}"
                                            class="text-brand-orange hover:underline font-bold">
                                            {{ $product->store->name }}
                                        </a>
                                    </span>
                                </div>
                            ...
                            <div class="space-y-8">
                                @forelse($product->productReviews as $review)
                                    <div class="flex gap-4 border-b border-gray-50 pb-8 last:border-0 last:pb-0">
                                        <div class="flex-shrink-0">
                                            <img src="https://ui-avatars.com/api/?name={{ $review->transaction->buyer->user->name ?? 'User' }}&background=random"
                                                alt="" class="w-12 h-12 rounded-full">
                                        </div>
                                        <div class="flex-1">
                                            <div class="flex justify-between items-start mb-2">
                                                <div>
                                                    <h4 class="font-bold text-brand-dark">
                                                        {{ $review->transaction->buyer->user->name ?? 'Anonymous' }}</h4>
                                ...
                                @empty
                                    <div class="text-center py-6">
                                        ...
                                        <h3 class="text-lg font-bold text-brand-dark mb-2">Belum Ada Ulasan</h3>
                                        <p class="text-gray-500">Jadilah yang pertama mengulas produk ini!</p>
                                    </div>
                                @endforelse
                            </div>
                        </div>
                                    ...
                                    <div class="flex flex-col justify-between py-1">
                                        <div>
                                            <h4 class="font-bold text-brand-dark leading-tight mb-1">
                                                <a href="{{ route('products.show', ['store' => $related->store->slug, 'product' => $related->slug]) }}"
                                                    class="hover:text-brand-orange transition-colors">{{ $related->name }}</a>
                                            </h4>
                                            <p class="text-xs text-brand-orange uppercase font-bold tracking-wider mb-2">
                                                {{ $related->productCategory->name ?? 'Kategori' }}</p>
                                            <p class="font-bold text-brand-dark">{{ $related->formatted_price }}</p>
                                        </div>
                                        @auth
                                            <button onclick="addToCart({{ $related->id }})"
                                                class="text-xs font-bold text-brand-orange border border-brand-orange rounded-lg px-3 py-1.5 hover:bg-brand-orange hover:text-white transition-colors flex items-center gap-1 w-max">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
