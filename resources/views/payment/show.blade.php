        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                    ...
                    <p class="text-gray-500 ml-9">ID Pesanan: <span
                            class="font-mono font-bold text-brand-dark">#{{ $transaction->code }}</span></p>
                </div>
            </div>

            @if(session('success'))
                <div
                    class="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl flex items-center shadow-sm mb-6">
                        ...
                        <div class="p-6">
                            @if($transaction->payment_status === 'waiting')
                                <div class="bg-blue-50 border border-blue-200 rounded-xl p-6 text-center">
                                    <div
                                        class="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-4 text-blue-600">
                                    ...
                                    <p class="text-blue-700">Bukti pembayaran Anda sedang diverifikasi oleh Admin. Mohon
                                        tunggu 1x24 jam.</p>
                                </div>
                            @elseif($transaction->payment_status === 'unpaid')
                                <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-6 text-center">
                                    ...
                                    <p class="text-yellow-700">Silakan selesaikan pembayaran Anda untuk memproses pesanan.
                                    </p>
                                </div>
                            @else
                            ...
                            <div class="p-6">
                                @if($transaction->payment_proof)
                                    <div class="mb-6">
                                        ...
                                        <div
                                            class="relative aspect-video w-full max-w-md rounded-xl overflow-hidden border border-gray-200 bg-gray-50">
                                            <img src="{{ asset('storage/' . $transaction->payment_proof) }}"
                                                class="w-full h-full object-contain">
                                        </div>
                                        <p class="text-xs text-gray-500 mt-2">Ingin mengganti bukti pembayaran? Silakan unggah
                                            file baru di bawah ini.</p>
                                    </div>
                                @endif

                                <form action="{{ route('payment.upload', $transaction->id) }}" method="POST"
                                    ...
                                    <div>
                                        ...
                                        <div
                                            class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-xl hover:border-brand-orange transition-colors group relative overflow-hidden">
                                                ...
                                                <div class="flex text-sm text-gray-600 justify-center">
                                                    <label for="payment_proof"
                                                        ...
                                                        <input id="payment_proof" name="payment_proof" type="file"
                                                            class="sr-only" accept="image/jpeg,image/png,image/jpg" required
                                            ...
                                            <div id="preview-container"
                                                class="hidden absolute inset-0 bg-white w-full h-full flex flex-col items-center justify-center p-2">
                                                <img id="preview-image" src="#" alt="Preview"
                                                    class="h-full w-auto object-contain max-h-[150px] rounded-lg shadow-sm">
                                                <button type="button" onclick="removeImage()"
                                                    class="absolute top-2 right-2 bg-red-100 text-red-600 rounded-full p-1 hover:bg-red-200 transition-colors">
                                                ...
                                                <p id="file-name"
                                                    class="text-xs text-gray-500 mt-2 truncate max-w-full px-4"></p>
                                            </div>
                                    ...
                                    <button type="submit"
                                        class="w-full bg-brand-dark text-white font-bold py-3 px-4 rounded-xl hover:bg-brand-orange transition-all duration-300 shadow-md transform hover:-translate-y-0.5">
                                ...
                                <div class="mt-6 bg-yellow-50 border border-yellow-200 rounded-xl p-4 flex gap-3">
                                    ...
                                    <p class="text-sm text-yellow-700">
                                        <strong>Catatan:</strong> Setelah mengunggah, mohon tunggu konfirmasi Admin. Anda
                                        akan diberitahu melalui email setelah pembayaran Anda diverifikasi.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="p-6">
                                ...
                                @foreach($transaction->transactionDetails as $detail)
                                    <div class="flex gap-4 items-center">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p class="font-bold text-sm text-brand-dark truncate">
                                                {{ $detail->product->name }}</p>
                                            <p class="text-xs text-gray-500">{{ $detail->qty }} x Rp
                                                {{ number_format($detail->product->price, 0, ',', '.') }}</p>
                                        </div>
                                        <div class="text-right">
                                            <p class="font-bold text-sm text-brand-dark">Rp
                            ...
                            <div class="border-t border-gray-100 pt-4 space-y-2">
                                <div class="flex justify-between text-sm text-gray-600">
                                    <span>Subtotal</span>
                                    <span>Rp
                                        {{ number_format($transaction->grand_total - $transaction->shipping_cost - $transaction->tax, 0, ',', '.') }}</span>
                                </div>
                                <div class="flex justify-between text-sm text-gray-600">
                                ...
                                <div
                                    class="border-t border-dashed border-gray-200 pt-4 mt-2 flex justify-between items-center">
                                    <span class="font-bold text-lg text-brand-dark">Total</span>
                                    <span class="font-bold text-xl text-brand-orange">Rp
