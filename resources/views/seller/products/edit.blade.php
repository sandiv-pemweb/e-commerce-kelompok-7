<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <form method="POST" action="{{ route('seller.products.update', $product) }}"
                                ...
                                <div>
                                    <label for="product_category_id"
                                        class="block font-bold text-sm text-gray-700 mb-2">Kategori</label>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label for="author"
                                            class="block font-bold text-sm text-gray-700 mb-2">Penulis</label>
                                    ...
                                    <div>
                                        <label for="publisher"
                                            class="block font-bold text-sm text-gray-700 mb-2">Penerbit</label>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label for="published_year"
                                            class="block font-bold text-sm text-gray-700 mb-2">Tahun Terbit</label>
                                        <input id="published_year" type="number" name="published_year"
                                            value="{{ old('published_year', $product->published_year) }}" min="1900"
                                            max="{{ date('Y') + 1 }}"
                                            class="w-full border-gray-300 focus:border-brand-orange focus:ring-brand-orange rounded-xl shadow-sm bg-gray-50/50"
                                    ...
                                    <div>
                                        <label for="condition"
                                            class="block font-bold text-sm text-gray-700 mb-2">Kondisi</label>
                                ...
                                <div>
                                    <label for="description"
                                        class="block font-bold text-sm text-gray-700 mb-2">Deskripsi</label>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label for="price" class="block font-bold text-sm text-gray-700 mb-2">Harga
                                            (Rp)</label>
                                    ...
                                    <div>
                                        <label for="discount_price"
                                            class="block font-bold text-sm text-gray-700 mb-2">Harga Diskon (Rp) <span
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        <label for="stock"
                                            class="block font-bold text-sm text-gray-700 mb-2">Stok</label>
                                    ...
                                    <div>
                                        <label for="weight" class="block font-bold text-sm text-gray-700 mb-2">Berat
                                            (gram)</label>
                                ...
                                <div class="pt-6 border-t border-gray-100 flex items-center gap-4">
                                    <a href="{{ route('seller.products.index') }}"
                                        class="px-6 py-3 bg-gray-100 text-gray-700 font-bold rounded-xl hover:bg-gray-200 transition-colors">Batal</a>
                                    <button type="submit"
                                        class="flex-1 px-6 py-3 bg-brand-dark text-white font-bold rounded-xl hover:bg-brand-orange transition-colors shadow-sm hover:shadow-md transform hover:-translate-y-0.5">Perbarui
                                        Produk</button>
                                </div>
                            </form>
                        </div>
                ...
                <div>
                    <div class="bg-white overflow-hidden shadow-sm rounded-3xl border border-gray-100">
                        <div class="p-8">
                            <h3 class="text-xl  font-bold text-brand-dark mb-6 border-b border-gray-100 pb-4">Gambar
                                Produk</h3>


                            <form method="POST" action="{{ route('seller.product-images.store', $product) }}"
                                enctype="multipart/form-data" class="mb-8">
                                @csrf
                                <div class="mb-4">
                                    <label for="images" class="block font-bold text-sm text-gray-700 mb-2">Tambah
                                        Gambar</label>
                                    <input id="images" type="file" name="images[]" multiple accept="image/*"
                                        class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-bold file:bg-brand-orange file:text-white hover:file:bg-brand-dark transition-colors cursor-pointer"
                                        required>
                                    @error('images')
                                        <span class="text-red-600 text-sm mt-1 block">{{ $message }}</span>
                                    @enderror
                                ...
                                <button type="submit"
                                    class="w-full px-4 py-2 bg-brand-dark text-white font-bold rounded-xl hover:bg-brand-orange transition-colors shadow-sm">Unggah
                                    Gambar</button>
                            </form>
                            ...
                            <div class="space-y-4">
                                @forelse($product->productImages as $image)
                                    <div class="relative group rounded-xl overflow-hidden shadow-sm border border-gray-100">
                                        <img src="{{ Str::startsWith($image->image, 'http') ? $image->image : asset('storage/' . $image->image) }}"
                                            alt="Product Image" class="w-full h-40 object-cover">
                                        <button type="button"
    ...
    @foreach($product->productImages as $image)
        <x-confirmation-modal id="delete-image-{{ $image->id }}" type="danger" title="Hapus Gambar"
            message="Apakah Anda yakin ingin menghapus gambar ini? Tindakan ini tidak dapat dibatalkan."
            confirmText="Ya, Hapus" cancelText="Batal">
        </x-confirmation-modal>
        <form id="delete-image-{{ $image->id }}-form" method="POST"
            action="{{ route('seller.product-images.destroy', [$product, $image]) }}" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    @endforeach
