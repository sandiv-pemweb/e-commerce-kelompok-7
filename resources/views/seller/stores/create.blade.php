    ...
    <div class="py-12 bg-brand-gray min-h-screen">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="mb-8 text-center">
                ...
                <p class="mt-2 text-gray-500 max-w-2xl mx-auto">Mulai perjalanan Anda sebagai penjual di platform kami.
                    Isi detail di bawah ini untuk membuat toko dan menjangkau lebih banyak pelanggan.</p>
            </div>
                    ...
                    <form method="POST" action="{{ route('seller.stores.store') }}" enctype="multipart/form-data"
                        ...
                        <div>
                            <h3 class="text-lg font-bold text-brand-dark mb-6 flex items-center gap-2">
                                <span
                                    class="w-8 h-8 bg-brand-orange text-white rounded-full flex items-center justify-center text-sm">1</span>
                                Detail Toko
                            </h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    <label for="name" class="block font-bold text-sm text-gray-700 mb-2">Nama
                                        Toko</label>
                                    <input id="name"
                                        class="w-full border-gray-300 focus:border-brand-orange focus:ring-brand-orange rounded-xl shadow-sm py-3 px-4"
                                        type="text" name="name" value="{{ old('name') }}" required autofocus
                                        placeholder="Contoh: Toko Buku Saya">
                                    @error('name')
                            ...
                            <div class="mt-6">
                                ...
                                <textarea id="about"
                                    class="w-full border-gray-300 focus:border-brand-orange focus:ring-brand-orange rounded-xl shadow-sm py-3 px-4"
                                    name="about" rows="4" required
                                    placeholder="Ceritakan sedikit tentang toko Anda dan apa yang Anda jual...">{{ old('about') }}</textarea>
                                @error('about')
                                    <span class="text-red-600 text-sm mt-1 block">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div>
                            <h3 class="text-lg font-bold text-brand-dark mb-6 flex items-center gap-2">
                                <span
                                    class="w-8 h-8 bg-brand-orange text-white rounded-full flex items-center justify-center text-sm">2</span>
                                Lokasi
                            </h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                            ...
                            <div class="mt-6">
                                ...
                                <textarea id="address"
                                    ...
                                    placeholder="Nama jalan, nomor bangunan, dll.">{{ old('address') }}</textarea>
                                @error('address')
                                    <span class="text-red-600 text-sm mt-1 block">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div>
                            ...
                            <div>
                                ...
                                <div
                                    class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-xl hover:border-brand-orange transition-colors group bg-gray-50 hover:bg-orange-50/30">
                        ...
                        <div class="flex items-center justify-end pt-8 border-t border-gray-100">
                            <button type="submit"
                                class="w-full md:w-auto px-8 py-4 bg-brand-dark text-white font-bold rounded-xl hover:bg-brand-orange transition-all duration-300 shadow-lg transform hover:-translate-y-1 text-lg">
                                Buat Toko
                            </button>
                        </div>
                    </form>
