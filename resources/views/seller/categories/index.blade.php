<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('seller.categories.create') }}"
                class="px-6 py-3 bg-brand-orange border border-transparent rounded-full font-bold text-sm text-white uppercase tracking-widest hover:bg-brand-dark transition-colors shadow-md hover:shadow-lg transform hover:-translate-y-0.5 flex items-center">
                ...
                <div class="p-8">
                    @if($categories->isEmpty())
                        <div class="text-center py-6">
                            ...
                            <a href="{{ route('seller.categories.create') }}"
                                class="inline-flex items-center px-6 py-3 bg-brand-dark border border-transparent rounded-xl font-bold text-sm text-white uppercase tracking-widest hover:bg-brand-orange transition-colors shadow-md">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                                </svg>
                                ...
                                <thead class="bg-brand-dark text-white">
                                    <tr>
                                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">Nama
                                            Kategori</th>
                                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">Jumlah
                                            Produk</th>
                                    ...
                                    @foreach($categories as $category)
                                        <tr class="hover:bg-blue-50 transition-colors duration-150">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <span
                                                    class="px-3 py-1 bg-gray-100 rounded-full text-xs font-bold text-gray-600">
                                                    {{ $category->products_count }} produk
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex space-x-2">
                                                <a href="{{ route('seller.categories.edit', $category) }}"
                                                    class="flex-1 text-center px-4 py-2 bg-gray-100 text-gray-700 font-bold rounded-xl hover:bg-brand-dark hover:text-white transition-colors">
                                                    Edit
                                                </a>
                                                <button type="button"
    ...
    @foreach($categories as $category)
        <x-confirmation-modal id="delete-category-{{ $category->id }}" type="danger" title="Hapus Kategori?"
        ...
        <form id="delete-category-{{ $category->id }}-form" method="POST"
            action="{{ route('seller.categories.destroy', $category) }}" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    @endforeach
