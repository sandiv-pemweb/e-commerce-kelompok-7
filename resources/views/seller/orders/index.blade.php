                ...
                <div class="p-8">
                    <form method="GET" action="{{ route('seller.orders.index') }}" class="flex flex-col md:flex-row gap-4 items-end">
                        <div class="w-full md:w-1/3">
                            ...
                            <select name="status" id="status" class="w-full border-gray-300 focus:border-brand-orange focus:ring-brand-orange rounded-xl shadow-sm py-2.5">
                        ...
                        <div class="w-full md:w-1/3">
                            ...
                            <select name="payment_status" id="payment_status" class="w-full border-gray-300 focus:border-brand-orange focus:ring-brand-orange rounded-xl shadow-sm py-2.5">
                        ...
                        <div class="w-full md:w-auto flex gap-3">
                            <button type="submit" class="px-6 py-2.5 bg-brand-dark text-white font-bold rounded-xl hover:bg-brand-orange transition-colors shadow-sm hover:shadow-md">Filter</button>
                        ...
                        <div class="overflow-x-auto rounded-2xl border border-gray-100">
                            <table class="min-w-full divide-y divide-gray-100">
                                <thead class="bg-gray-50/50">
                                    <tr>
                                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider pl-8">ID Pesanan</th>
                                        ...
                                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Status Pesanan</th>
                                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Status Pembayaran</th>
                                    ...
                                    @foreach($orders as $order)
                                        <tr class="hover:bg-blue-50/50 transition-colors duration-200 group">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    <div class="flex-shrink-0 h-9 w-9">
                                                        <img class="h-9 w-9 rounded-full border border-gray-100 object-cover" src="https://ui-avatars.com/api/?name={{ urlencode($order->buyer->user->name) }}&background=random" alt="">
                                                    </div>
                                                    <div class="ml-3">
                                                        <div class="text-sm font-bold text-gray-900">{{ $order->buyer->user->name }}</div>
                                                        <div class="text-xs text-gray-500">{{ $order->buyer->user->email }}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm font-bold text-brand-orange">Rp {{ number_format($order->grand_total, 0, ',', '.') }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium pr-8">
                                                <a href="{{ route('seller.orders.show', $order) }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-700 hover:bg-brand-dark hover:text-white hover:border-brand-dark transition-all shadow-sm group-hover:shadow-md">
                                                    Detail
                                                </a>
                                            </td>
                                        </tr>
