    ...
    <div class="py-6">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-3xl border border-gray-100">
                <div class="p-8">
                    <!-- Balance Info -->
                    <div class="mb-8 p-8 bg-gradient-to-br from-blue-50 to-white rounded-2xl border border-blue-100 flex items-center justify-between">
                        <div>
                            <p class="text-xs font-bold text-blue-600 uppercase tracking-wider mb-2">Saldo Tersedia Untuk Ditarik</p>
                            <p class="text-4xl font-bold text-brand-dark">Rp {{ number_format($storeBalance->balance, 0, ',', '.') }}</p>
                        </div>
                        <div class="h-16 w-16 bg-blue-100 rounded-2xl flex items-center justify-center text-blue-600 shadow-inner">
                    ...
                    @if($store->bank_name && $store->bank_account_name && $store->bank_account_number)
                        <div class="mb-8 p-6 bg-gray-50 rounded-2xl border border-gray-100">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div>
                                    <span class="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-1">Bank</span>
                                    <span class="text-gray-900 font-bold text-lg">{{ $store->bank_name }}</span>
                                </div>
                                <div>
                                    <span class="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-1">Pemilik Rekening</span>
                                    <span class="text-gray-900 font-bold text-lg">{{ $store->bank_account_name }}</span>
                                </div>
                                <div>
                                    <span class="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-1">Nomor Rekening</span>
                                    <span class="text-gray-900 font-bold text-lg font-mono">{{ $store->bank_account_number }}</span>
                                </div>
                            </div>
                            <p class="text-xs text-gray-400 mt-6 pt-4 border-t border-gray-200 flex items-center gap-1">
                                ...
                                Info: Anda dapat mengubah informasi rekening bank di <a href="{{ route('seller.stores.edit') }}" class="text-brand-orange hover:text-brand-dark font-bold transition-colors ml-1">Profil Toko</a>
                            </p>
                        </div>
                    @else
                        <div class="mb-8 p-6 bg-yellow-50 border border-yellow-200 rounded-2xl flex items-start shadow-sm">
                            <div class="p-2 bg-yellow-100 rounded-lg mr-3">
                                <svg class="w-6 h-6 text-yellow-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                            </div>
                            <div>
                                <h4 class="font-bold text-yellow-800 mb-1 text-lg">Informasi Bank Tidak Lengkap</h4>
                                <p class="text-sm text-yellow-700 leading-relaxed">Harap lengkapi informasi rekening bank Anda di <a href="{{ route('seller.stores.edit') }}" class="font-bold underline decoration-2 decoration-yellow-400 hover:text-yellow-900">Profil Toko</a> sebelum mengajukan penarikan.</p>
                            </div>
                        </div>
                    @endif
