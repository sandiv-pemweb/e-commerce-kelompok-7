<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            @if (session('success'))
                <div
                    class="mb-6 bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl flex items-center shadow-sm">
            ...
            @if (session('error'))
                <div
                    class="mb-6 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl flex items-center shadow-sm">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-3xl border border-gray-100 relative">
                ...
                <div class="p-8 md:p-12">
                    ...
                    <div class="text-center mb-10">
                        ...
                        <p class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Permintaan Penarikan
                        </p>
                    ...
                    <div class="flex items-center gap-6 mb-8 bg-gray-50/50 p-6 rounded-3xl border border-gray-100">
                        @if($withdrawal->storeBalance->store->logo)
                            <img class="h-16 w-16 rounded-2xl object-cover border border-white shadow-sm"
                                src="{{ $withdrawal->storeBalance->store->logo_url }}"
                                alt="{{ $withdrawal->storeBalance->store->name }}">
                        @else
                            <div
                                class="h-16 w-16 rounded-2xl bg-white flex items-center justify-center text-xl font-bold text-gray-400 border border-gray-200">
                                {{ substr($withdrawal->storeBalance->store->name, 0, 1) }}
                            </div>
                        @endif
                        <div>
                            <h4 class="text-xl font-bold text-brand-dark mb-1">
                                {{ $withdrawal->storeBalance->store->name }}</h4>
                            <p class="text-gray-500 text-sm mb-2">{{ $withdrawal->storeBalance->store->user->name }} •
                                {{ $withdrawal->storeBalance->store->user->email }}</p>
                            <div
                                class="mt-1 text-xs font-bold text-green-700 bg-green-50 inline-block px-3 py-1 rounded-full border border-green-100">
                    ...
                    <div class="bg-gray-50/50 rounded-3xl p-8 border border-gray-100 mb-8">
                        <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-6">Informasi Rekening
                            Tujuan</h4>
                        <div class="grid grid-cols-2 gap-8">
                            ...
                            <div class="col-span-2 pt-6 border-t border-gray-200/50 mt-2">
                                <p class="text-xs text-gray-400 mb-2 uppercase tracking-wider font-bold">Nomor Rekening
                                </p>
                                <p class="font-mono font-bold text-brand-dark text-2xl tracking-wider">
                                    {{ $withdrawal->bank_account_number }}</p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-gray-50/50 px-8 py-4 border-t border-gray-100 text-center">
                    <p class="text-xs text-gray-400 font-mono">REF ID:
                        {{ str_pad($withdrawal->id, 8, '0', STR_PAD_LEFT) }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    @if($withdrawal->status == 'pending')
        ...
        <x-confirmation-modal id="approve-modal" type="success" title="Konfirmasi Transfer"
            ...
            confirmText="Ya, Saya Sudah Transfer" cancelText="Batal">
        </x-confirmation-modal>
        <form id="approve-modal-form" method="POST" action="{{ route('admin.withdrawals.approve', $withdrawal) }}"
            class="hidden">
            @csrf
            @method('PATCH')
        </form>
        ...
        <x-confirmation-modal id="reject-modal" type="danger" title="Tolak Penarikan"
            message="Apakah Anda yakin ingin menolak permintaan penarikan ini? Saldo akan dikembalikan ke akun toko."
            confirmText="Tolak Permintaan" cancelText="Batal">
        </x-confirmation-modal>
        <form id="reject-modal-form" method="POST" action="{{ route('admin.withdrawals.reject', $withdrawal) }}"
            class="hidden">
            @csrf
            @method('PATCH')
        </form>
