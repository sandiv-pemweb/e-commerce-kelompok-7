<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            @if (session('success'))
                <div
                    class="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-2xl flex items-center shadow-sm">
            ...
            @if (session('info'))
                <div
                    class="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-xl flex items-center shadow-sm">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-3xl border border-gray-100">
                <div class="p-8">
                    <div class="flex items-center justify-between mb-8 border-b border-gray-100 pb-6">
                        ...
                        <span
                            class="px-4 py-2 inline-flex items-center gap-1.5 text-sm leading-5 font-bold rounded-full {{ $store->is_verified ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-yellow-50 text-yellow-700 border border-yellow-100' }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        <div class="md:col-span-1">
                            @if($store->logo)
                                <img src="{{ $store->logo_url }}" alt="{{ $store->name }}"
                                    class="w-full h-auto object-cover rounded-3xl shadow-sm border border-gray-100">
                            @else
                                <div
                                    class="w-full h-64 bg-gray-50 rounded-3xl flex items-center justify-center text-gray-400 border border-gray-100">
                        ...
                        <div class="md:col-span-2 space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Nama Toko
                                    </p>
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <a href="{{ route('admin.users.show', $store->user) }}"
                                            class="text-xs font-bold text-brand-orange hover:text-brand-dark transition-colors border border-brand-orange/20 px-2 py-0.5 rounded-full hover:bg-brand-orange/10">Lihat
                                            User</a>
                                    </div>
                                ...
                                <div>
                                    <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Total
                                        Produk</p>
                                    <p class="text-lg font-medium text-gray-900">{{ $store->products->count() }} produk
                                    </p>
                                </div>
                                <div class="md:col-span-2">
                                    <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Alamat</p>
                                    <p class="text-lg font-medium text-gray-900">{{ $store->address }},
                                        {{ $store->city }}, {{ $store->postal_code }}</p>
                                </div>
                                <div class="md:col-span-2">
                                    <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Tentang
                                        Toko</p>
                                    <p
                                        class="text-base text-gray-600 leading-relaxed bg-gray-50 p-4 rounded-3xl border border-gray-100">
                                        {{ $store->about }}</p>
                                </div>
                                @if($store->storeBalance)
                                    <div class="md:col-span-2">
                                        <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Saldo Toko
                                        </p>
                                        <p class="text-3xl font-bold text-brand-orange">Rp
                                            {{ number_format($store->storeBalance->balance, 0, ',', '.') }}</p>
                                    </div>
                                @endif
                            </div>
    ...
    @if(!$store->is_verified)
        <x-confirmation-modal id="verify-modal" type="success" title="Verifikasi Toko"
        ...
        <x-confirmation-modal id="reject-modal" type="danger" title="Tolak Pengajuan Toko"
            message="Apakah Anda yakin ingin menolak dan menghapus {{ $store->name }}? Tindakan ini tidak dapat dibatalkan."
            confirmText="Ya, Tolak & Hapus" cancelText="Batal">
        </x-confirmation-modal>
    ...
    <x-confirmation-modal id="delete-modal" type="danger" title="Hapus Toko"
