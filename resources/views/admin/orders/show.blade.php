        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                <a href="{{ route('admin.orders.index') }}"
                    class="p-2 bg-white rounded-xl border border-gray-200 hover:bg-gray-50 text-gray-500 hover:text-brand-dark transition-colors">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-400 mt-1 font-medium">Dipesan pada
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    @if($order->tracking_number)
                        <div
                            class="bg-gradient-to-r from-blue-50 to-white border border-blue-100 rounded-3xl p-6 flex items-start gap-4">
                    ...
                    <div class="bg-white overflow-hidden shadow-sm rounded-3xl border border-gray-100">
                        <div class="p-8 border-b border-gray-50 flex items-center justify-between">
                            ...
                            <span class="text-sm text-gray-400 font-bold">{{ $order->transactionDetails->count() }}
                                Item</span>
                        </div>
                        <div class="p-8 space-y-8">
                            @foreach($order->transactionDetails as $detail)
                                <div class="flex gap-6 items-start">
                                    <div
                                        class="w-24 h-24 flex-shrink-0 bg-gray-50 rounded-2xl overflow-hidden border border-gray-100">
                                    ...
                                    <div class="flex-1 min-w-0 py-1">
                                        ...
                                        <p class="text-sm text-gray-500 mb-1 font-medium">{{ $detail->qty }} x Rp
                                            {{ number_format($detail->product->price, 0, ',', '.') }}</p>
                                        <p class="text-xs text-brand-orange font-bold uppercase tracking-wider">
                                            {{ $detail->product->productCategory->name ?? 'General' }}</p>
                                    </div>
                        ...
                        <div class="bg-gray-50/50 p-8 border-t border-gray-50 space-y-4">
                            <div class="flex justify-between text-sm">
                                ...
                                <span class="font-bold text-gray-700">Rp
                                    {{ number_format($order->grand_total - $order->shipping_cost - $order->tax, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between text-sm">
                            ...
                            <div class="pt-6 border-t border-gray-200 mt-2 flex justify-between items-center">
                                ...
                                <span class="font-bold text-3xl text-brand-orange">Rp
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white shadow-sm rounded-3xl border border-gray-100 p-8">
                        <div class="flex justify-between items-start mb-6">
                            ...
                            <a href="{{ route('admin.stores.show', $order->store) }}"
                                class="text-xs font-bold text-brand-orange hover:text-brand-dark transition-colors">Lihat
                                Detail &rarr;</a>
                        </div>
                        <div class="flex items-center gap-4">
                            @if($order->store->logo)
                            ...
                            @else
                                <div
                                    class="w-12 h-12 rounded-full bg-brand-orange text-white flex items-center justify-center text-lg font-bold border-2 border-orange-100 shadow-sm">
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2 mt-1">
                                    @if($order->store->is_verified)
                                        <span
                                            class="px-2 py-0.5 bg-green-100 text-green-700 text-[10px] font-bold rounded-full border border-green-200">Verified</span>
                                    @else
                                        <span
                                            class="px-2 py-0.5 bg-yellow-100 text-yellow-700 text-[10px] font-bold rounded-full border border-yellow-200">Unverified</span>
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-white shadow-sm rounded-3xl border border-gray-100 p-8">
                        ...
                        <div class="space-y-6">
                            <div class="flex items-center gap-4">
                                <img src="https://ui-avatars.com/api/?name={{ $order->buyer->user->name }}&background=random"
                            ...
                            <div class="bg-gray-50 rounded-2xl p-4 border border-gray-100">
                                <p class="text-xs font-bold text-gray-400 uppercase mb-2 tracking-wider">Alamat
                                    Pengiriman</p>
                        ...
                        @if($order->payment_proof)
                            <div class="mb-6">
                                ...
                                <a href="{{ asset('storage/' . $order->payment_proof) }}" target="_blank"
                                    class="block group relative overflow-hidden rounded-2xl border border-gray-200">
                                    ...
                                    <div
                                        class="absolute inset-0 bg-brand-dark/0 group-hover:bg-brand-dark/20 transition-colors flex items-center justify-center">
                                        <div
                                            class="bg-white px-4 py-2 rounded-full text-xs font-bold text-brand-dark shadow-lg opacity-0 group-hover:opacity-100 transition-all transform translate-y-4 group-hover:translate-y-0">
                                            Perbesar Gambar
                                        </div>
                                    </div>
                                </a>
                            ...
                            @if($order->payment_status === 'unpaid' || $order->payment_status === 'waiting')
                                ...
                                <form id="verify-payment-{{ $order->id }}-form"
                                    action="{{ route('admin.orders.verify-payment', $order) }}" method="POST" class="hidden">
                                    @csrf
                                    @method('PATCH')
                                </form>
                                <x-confirmation-modal id="verify-payment-{{ $order->id }}" type="info"
                                    title="Verifikasi Pembayaran"
                                    message="Pastikan bukti transfer valid. Verifikasi akan mengubah status menjadi Lunas."
                                    confirmText="Ya, Verifikasi Valid" cancelText="Batal">
