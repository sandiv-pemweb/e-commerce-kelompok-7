                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    @foreach($wishlists as $item)
                        <div class="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow overflow-hidden group">
                            ...
                            <div class="p-4">
                                <div class="text-xs text-brand-orange font-medium mb-1">
                                    {{ $item->product->productCategory->name ?? 'Tidak Berkategori' }}</div>
                                <h3 class="font-bold text-brand-dark truncate mb-1">
                                    <a
                                        href="{{ route('products.show', ['store' => $item->product->store->slug, 'product' => $item->product->slug]) }}">{{ $item->product->name }}</a>
                                </h3>
